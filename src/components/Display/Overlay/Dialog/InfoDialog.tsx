...
function InfoDialog(props: Props) {
    ...
    return <Overlay
        open={open}
        onToggle={onToggle}
        className={'max-w-[92.5vw] sm:max-w-sm md:max-w-md min-h-[180px] md:min-h-[200px]'}
        overlayAttrs={{
            'aria-labelledby': id
        }}
    >
        <div
            className={
                className([
                    'rounded-md transition-opacity bg-white dark:bg-gray-800 shadow-2xl dark:shadow-none dark:border default-border-color border-opacity-0 dark:border-opacity-40 transition-slow',
                    {
                        'opacity-0': !open
                    }
                ])
